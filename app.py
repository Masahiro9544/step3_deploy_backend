...
@app.get("/allwords")
def read_all_word():
    result = crud.myselectAll(mymodels.Words)
    # 結果がNoneの場合は空配列を返す
    if not result:
        return []
...
@app.get("/words")
def read_one_word(id: int):
    result = crud.onewordselect(mymodels.Words, id)
    if not result:
...
@app.post("/registword")
def register_word(data: Word):
    values = data.dict()
    result = crud.myinsert(mymodels.Words, values)
